...
ActionApplier::ActionApplier(const QList<AlpmPackage *> & remove_all_pkgs,const QList<AlpmPackage *> & remove_pkgs,const QList<AlpmPackage *> & install_pkgs,const QList<AlpmPackage *> & installdeps_pkgs,const QList<AlpmPackage *> & installforced_pkgs,ProgressView * view,QAction * cancelAction,OptionalDepsDlg * optdlg,QObject *parent) : PackageProcessorBase(parent) {
    ...
    connect(this,SIGNAL(completed(ThreadRun::RC)),this,SLOT(deleteLater()),Qt::QueuedConnection);
    ...
    if (remove_all_pkgs.count() > 0) {
        ...
        connect(processor,SIGNAL(completed(ThreadRun::RC)),SLOT(remove_all_completed(ThreadRun::RC)),Qt::QueuedConnection);
        ...
        connect(processor,SIGNAL(completed(ThreadRun::RC)),SLOT(remove_completed(ThreadRun::RC)),Qt::QueuedConnection);
        connect(processor,SIGNAL(logString(const QString &)),this,SIGNAL(logString(const QString &)));
        connect(processor,SIGNAL(canceled()),SIGNAL(canceled()));
        m_install_pkgs = install_pkgs;
        m_installdeps_pkgs = installdeps_pkgs;
        ...
        connect(processor,SIGNAL(completed(ThreadRun::RC)),SLOT(install_completed(ThreadRun::RC)),Qt::QueuedConnection);
        connect(processor,SIGNAL(logString(const QString &)),this,SIGNAL(logString(const QString &)));
        connect(processor,SIGNAL(canceled()),SIGNAL(canceled()));
        m_installdeps_pkgs = installdeps_pkgs;
    }
        ...
        connect(processor,SIGNAL(completed(ThreadRun::RC)),SIGNAL(completed(ThreadRun::RC)),Qt::QueuedConnection);
        connect(processor,SIGNAL(logString(const QString &)),this,SIGNAL(logString(const QString &)));
        connect(processor,SIGNAL(canceled()),SIGNAL(canceled()));
    }
    else QMetaObject::invokeMethod(this,"completed",Qt::QueuedConnection,Q_ARG(ThreadRun::RC,ThreadRun::BAD));
}
...
void ActionApplier::split_forced_pkg(const QList<AlpmPackage *> & installforced_pkgs) {
    ...
    for (int i=0;i<installforced_pkgs.count();i++) {
        AlpmPackage * pkg = installforced_pkgs[i];
...
void ActionApplier::install_completed(ThreadRun::RC ok) {
    if (ok !=ThreadRun::OK) {
        emit completed(ok);
        return;
    }
    ...
    else emit completed(ok);
}

void ActionApplier::remove_all_completed(ThreadRun::RC ok) {
    if (ok !=ThreadRun::OK) {
        emit completed(ok);
        return;
    }
...
void ActionApplier::remove_completed(ThreadRun::RC ok) {
    if (ok !=ThreadRun::OK) {
        emit completed(ok);
        return;
    }
