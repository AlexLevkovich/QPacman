...
DBExtObject::DBExtObject(QTextEdit * edit,const QString & text) : WidgetTextObject(edit) {
    ...
    for (QString ext: AlpmConfig::dbExtensions()) {
        combo->addItem(ext);
    }

    for (int i=0;i<combo->count();i++) {
        if (combo->itemText(i) == AlpmConfig::dbExtension()) {
            combo->setCurrentIndex(i);
            break;
        }
    }
...
InfoView::InfoView(QWidget *parent) : CustomPopupTextBrowser(parent) {
    ...
    insertText(table->cellAt(1,1).firstCursorPosition(),config.rootDir());
    insertText(table->cellAt(2,0).firstCursorPosition(),"DBPath");
    insertText(table->cellAt(2,1).firstCursorPosition(),config.dbPath());
    insertText(table->cellAt(3,0).firstCursorPosition(),"GPGDir");
    insertText(table->cellAt(3,1).firstCursorPosition(),config.gpgDir());
    insertText(table->cellAt(4,0).firstCursorPosition(),"LogFile");
    insertText(table->cellAt(4,1).firstCursorPosition(),config.logFileName());
    insertText(table->cellAt(5,0).firstCursorPosition(),"Architecture");
    insertText(table->cellAt(5,1).firstCursorPosition(),config.arch());
    insertText(table->cellAt(6,0).firstCursorPosition(),"UseSyslog");
    insertText(table->cellAt(6,1).firstCursorPosition(),config.doUseSysLog()?"1":"0");
    insertText(table->cellAt(7,0).firstCursorPosition(),"DisableDownloadTimeout");
    insertText(table->cellAt(7,1).firstCursorPosition(),config.doDisableDownloadTimeout()?"1":"0");
    insertText(table->cellAt(8,0).firstCursorPosition(),"SigLevel");
    insertText(table->cellAt(8,1).firstCursorPosition(),config.sigLevel().join(" "));
    insertText(table->cellAt(9,0).firstCursorPosition(),"LocalFileSigLevel");
    insertText(table->cellAt(9,1).firstCursorPosition(),config.localFileSigLevel().join(" "));
    insertText(table->cellAt(10,0).firstCursorPosition(),"RemoteFileSigLevel");
    insertText(table->cellAt(10,1).firstCursorPosition(),config.remoteFileSigLevel().join(" "));
    ...
    insertText(table->cellAt(15,1).firstCursorPosition(),config.ignorePkgs().join(" "));
    insertText(table->cellAt(16,0).firstCursorPosition(),"NoExtract");
    insertText(table->cellAt(16,1).firstCursorPosition(),config.noExtractPkgs().join(" "));
    insertText(table->cellAt(17,0).firstCursorPosition(),"NoUpgrade");
    insertText(table->cellAt(17,1).firstCursorPosition(),config.noUpgradePkgs().join(" "));
    setupTableHeader(table,palette().color(QPalette::Active,QPalette::Mid),palette().buttonText().color());
    setTableRowColors(table,palette().button().color(),viewport()->palette().color(QPalette::Active,QPalette::Base));
    textCursor().insertBlock();
    textCursor().insertBlock();
    ...
    useSysIconsBox = new CheckBoxTextObject(this,tr("Use the system icons"),AlpmConfig::useSystemIcons());
    useSysIconsBox->insert(table->cellAt(1,0).firstCursorPosition());
    extObj = new DBExtObject(this,tr("Database files' suffix (restart needed)"));
    extObj->insert(table->cellAt(2,0).firstCursorPosition());
    threadsObj = new LabelSpinTextObject(this,tr("Count of the threads for each download"),1,20,AlpmConfig::downloaderThreadCount());
    threadsObj->insert(table->cellAt(3,0).firstCursorPosition());
    timeoutObj = new LabelSpinTextObject(this,tr("Connection timeout"),0,300,AlpmConfig::downloaderTimeout()/1000,tr(" secs"));
...
void InfoView::hideEvent(QHideEvent *event) {
    ...
    AlpmConfig::setDownloaderTimeout(timeoutObj->value()*1000);
    AlpmConfig::setDownloaderThreads(threadsObj->value());
    AlpmConfig::setUsingSystemIcons(useSysIconsBox->isChecked());
    AlpmConfig::setDBExtension(extObj->value());
