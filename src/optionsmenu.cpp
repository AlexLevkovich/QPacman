...
QStringList InstallOptionsMenu::titleTexts() const {
    return QStringList() << OptionsWidget::statusText(AlpmPackage::DO_INSTALL);
}
...
QStringList ReinstallOptionsMenu::titleTexts() const {
    return QStringList() << OptionsWidget::statusText(AlpmPackage::DO_UNINSTALL) << OptionsWidget::statusText(AlpmPackage::DO_REINSTALL);
}
...
QStringList UninstallOptionsMenu::titleTexts() const {
    return QStringList() << OptionsWidget::statusText(AlpmPackage::DO_UNINSTALL);
}
...
void OkCancelTitleBar::setIcon(const QIcon & icon) {
    iconLabel->setPixmap(icon.pixmap(Static::quadroSize(fontMetrics().height()+4)));
}
