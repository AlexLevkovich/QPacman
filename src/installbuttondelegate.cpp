...
QSize InstallButtonDelegate::sizeHint(const QStyleOptionViewItem & option,const QModelIndex & index) const {
    ...
    PacmanView * view = qobject_cast<PacmanView*>((QObject *)option.widget);
    ...
    PacmanItemModel * model = (PacmanItemModel *)index.model();
...
bool InstallButtonDelegate::editorEvent(QEvent *event,QAbstractItemModel *_model,const QStyleOptionViewItem &option,const QModelIndex &index) {
    ...
    if(((event->type() == QEvent::MouseButtonRelease) && (index.column() == lastColumn)) || ((event->type() == QEvent::KeyPress) && (((QKeyEvent *)event)->modifiers() & Qt::ShiftModifier) && (((QKeyEvent *)event)->key() == Qt::Key_Right))) {
         ...
         if (isArrowPressed) {
            ...
            qRegisterMetaType<PacmanView *>("PacmanView *");
            QMetaObject::invokeMethod(this,"show_menu",Qt::QueuedConnection,Q_ARG(AlpmPackage *,model->row(index)),Q_ARG(QModelIndex,index),Q_ARG(QRect,option.rect),Q_ARG(PacmanView *,view));
         }
         else {
...
void InstallButtonDelegate::show_menu(AlpmPackage * pkg,const QModelIndex & index,const QRect & rect,PacmanView * view) {
    QMenu * menu = NULL;
...
void InstallButtonDelegate::paint(QPainter *painter, const QStyleOptionViewItem &_option, const QModelIndex &index) const {
    ...
    if (option.state & QStyle::State_Selected) {
        ...
        PacmanView * view = qobject_cast<PacmanView*>((QObject *)option.widget);
        if (view == NULL) return;
        int row = view->visibleRowIndex(model->index(index.row(),0));
    ...
    if(index.column() != lastColumn) {
         if(index.column() == 0) {
             painter->drawPixmap(0,option.rect.y(),model->data(index,Qt::DecorationRole).value<QIcon>().pixmap(Static::quadroSize(option.fontMetrics.height()+4)));
...
void InstallButtonDelegate::item_selected(const QItemSelection & selection,PacmanView * view) {
...
bool InstallButtonDelegate::eventFilter(QObject *editor,QEvent *event) {
     PacmanView * view = qobject_cast<PacmanView*>(editor);
